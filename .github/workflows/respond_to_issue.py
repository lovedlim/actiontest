...
# ChatGPT API 호출
def get_chatgpt_response(comment_body):
    try:
        prompt = f"User {COMMENT_AUTHOR} asked: {comment_body}"
...
def main():
    if COMMENT_BODY:  # 댓글 내용이 있을 때만 응답 생성
        response = get_chatgpt_response(COMMENT_BODY)
